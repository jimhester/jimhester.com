...
```{r, include = FALSE, cache = TRUE}
## Get pkg releases
get_pkgs <- function(search, size = 100) {
  url <- str_glue("http://seer.r-pkg.org:9200/_search?q=Maintainer:{search}&size={size}")

  res <- jsonlite::read_json(url)

  map_chr(res$hits$hits, list("_source", "Package"))
}

pkgs <- get_pkgs("Hester")
...
pkg_data <- get_releases(get_pkgs("Hester")) %>%
  mutate(
    type = "pkg",
...
p_data <- filter(all_data, date > today() - years(1)) %>%
  mutate(
...
counts <- count(p_data, type, .drop = FALSE) %>%
  ...
  mutate(
    n = case_when(
      type == "github" ~ count(p_data, type, wt = as.integer(count)) %>% pull(),
      TRUE ~ n),
    emoji = c("💻", "📹", "📋", "📽", "⚒", "📦"),
...
library(ggiraph)
p <- ggplot() +
  geom_tile(data = filter(p_data, is.na(type)), width = 6, height = .8, aes(x, y, fill = type)) +
  geom_tile_interactive(data = filter(p_data, !is.na(type)), width = 6, height = .8, aes(x, y, fill = type, alpha = log(as.numeric(count)), tooltip = tip, onclick = on_click, data_id = data_id)) +
  scale_fill_manual(
  ...
  guides(
    alpha = "none",
    fill = guide_legend(NULL, label.theme = element_text(hjust = .3, family = "Lato", size = 7),
      label.position = "right", keywidth = unit(.6, "cm"), keyheight = unit(.25, "cm")
    )
  ) +
  labs(x = NULL, y = NULL)
